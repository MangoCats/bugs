...
#define WATERSTART      100   /* world starts uniformly covered in water, which is conserved           */
  ...
  for ( i = 0 ; i < NSENSECELLS ; i++ )
    { // navigate to the cells
      ...
      switch ( i )
       ...
       sense[i+NSENSECELLS*4] = (world[cp.x][cp.y].water * 1024) / bug->pos[0].weight;  // Water depth, normalized to bug weight
...
{             long  i,j,mass,face,ngenes,wetness,watercons;
  ...
  switch( i )
	...
	if ( bug->pos[0].hydrate < (bug->pos[0].weight / 1024) )  // Can't drink when already full
	  { if ( world[bug->pos[0].p.x][bug->pos[0].p.y].water > 0 )  // Can't drink when there's no water
	      { // Try to drink it all!
	        bug->pos[0].hydrate += world[bug->pos[0].p.x][bug->pos[0].p.y].water;
		world[bug->pos[0].p.x][bug->pos[0].p.y].water = 0;
		if ( bug->pos[0].hydrate > (bug->pos[0].weight / 1024) )  // Over full?
		  { // Give some back
		    world[bug->pos[0].p.x][bug->pos[0].p.y].water = bug->pos[0].hydrate - (bug->pos[0].weight / 1024);
  ...
  if ( bug != NULL )   // could have been killed in a fight
    if (( bug->pos[0].weight  < DIETHIN ) || 
        ( bug->pos[0].hydrate <= 0      ) || 
	( bug->pos[0].weight / 100 < world[bug->pos[0].p.x][bug->pos[0].p.y].water )) // starve, thirst, or drown
      { if ( bug->pos[0].weight / 100 < world[bug->pos[0].p.x][bug->pos[0].p.y].water )
          hist[today % LHIST].drownings++;
	 else 
          hist[today % LHIST].starvations++;
        kill_bug( bug ); // too thin or dry or underwater to live, feed the grass
      }
    ...
    { switch ( i )
        { case 0:
          default:                 // shouldn't be used... 
            bug->brain.act[i].a = add_gene( 5, 81, 1215, 1085, NULL               , 0 );
            bug->brain.act[i].a = add_gene( 3, 81, 1217, 1083, bug->brain.act[i].a, 0 );
            bug->brain.act[i].a = add_gene( GENECONST, NSENSECELLS + 1, 1501, 1047, bug->brain.act[i].a, 1 );
            bug->brain.act[i].b = add_gene( 3, 81, 1204, 1057, NULL               , 0 );
            bug->brain.act[i].b = add_gene( GENECONST, NSENSECELLS + 1, 1999, 1049, bug->brain.act[i].b, 1 );
            // bug->brain.act[i].a = add_gene( 1, 79,  26,  363, NULL, 0 );
            // bug->brain.act[i].b = add_gene( 1, 79,  63, 1530, NULL, 0 );
            break;
...
//      if ( today == 294912 ) interval = 1024;  // Begin year 9
      ...
      if ( wait > 0 )
        ...
        { if (( stage == 0 ) && (buglist.n_bugs > 1000)) { foodhump = 2.0; stage = 1; wait =   0; }
