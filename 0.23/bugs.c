            ...
            { age = bugs = mass = kills = genes = 0;
              ...
              if ( y > 0 )
                { gdImageLine( im, (lastbugs  * LEFTBAR) / maxbugs , y - 1, (bugs  * LEFTBAR) / maxbugs , y, gdTrueColor( 255,255,  0 ) ); // Population in yellow
                  ...
                  gdImageLine( im, (lastgenes * LEFTBAR) / maxgenes, y - 1, (genes * LEFTBAR) / maxgenes, y, gdTrueColor(   0,255,  0 ) ); // Genes in green
                }
              lastage  = age;       
              lastbugs = bugs;      
              lastmass = mass;      
...
//      if ( today == 294912 ) interval = 1024;  // Begin year 9
      ...
      if ( wait > 0 )
        ...
        { if (( stage == 0 ) && (buglist.n_bugs > 10000)) { safety = 0; stage = 1; wait =   0; }
          if (( stage == 1 ) && (buglist.n_bugs > 20000)) { leak   = 0; stage = 2; wait = 250; }
          
          // Could use stage 2 & beyond for other things, but the forcemate escalation is somewhat population controlled using agediv
